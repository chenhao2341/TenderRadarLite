...
class HengyangXwdtAdapter(BaseAdapter):
    def parse(self, html: str) -> List[Notice]:
        notices: List[Notice] = []
        for title, full_url, published_at in find_links_with_dates(html, self.url, "/xwzx/xwdt/"):
            notices.append(
                Notice(
                    source=self.source_config.get("source", self.source_name),
                    source_subtype=self.source_config.get("source_subtype", "中心要闻"),
