...
- `v0.1-alpha`：本地报告、双 supported 来源、默认关闭 alpha 来源、Source Catalog、Source Quality Matrix、字段质量审计、Windows 本地运行、Web 控制台真实运行入口、安全边界说明
- `v0.2-alpha`：Web 控制台进一步硬化与更多工作流
- `v0.3-beta`：Source Probe / 更多异构来源 / 质量持续修复

## 开源发布边界

- 当前开源主路径是命令行 / Windows 脚本 / 本地 HTML 报告
- Web 控制台可作为真实本地运行入口，但仍处于 Alpha / Developer Preview 边界内
- 不承诺所有来源字段完美
- 不承诺全国稳定覆盖
- 不承诺附件深度解析
- 不承诺自动投标、中标预测、SaaS、生产可用或登录 / 验证码绕过能力
- Source Catalog 与 Source Quality Matrix 用于说明来源状态和质量边界，不代表全国全量覆盖或成熟商业化能力
- 用户应自行遵守目标网站条款、robots、访问频率和当地法律法规
- 数据内容归原始公告发布网站所有
